...
// 仅修改类名：ActorController → ActorControllerui（最小改动）
public class ActorControllerui : MonoBehaviour
{
    public GameObject model;//抓取要控制的模型
    // 关键修正：这里的类型要从 PlayerInput 改为 PlayerInputui（和你Awake里的引用匹配）
    public PlayerInputui pi;//调用PlayerInputui脚本
    [SerializeField]
    ...
    void Awake()
    {
        ...
        // 你这里已经改对了：GetComponent<PlayerInputui>()
        pi = GetComponent<PlayerInputui>();
